...
const LabourWelfareFundTab = ({ lwfData, enabled, onEnable, onDisable, loading }) => {
  const [stateInput, setStateInput] = useState(lwfData?.state || "");
  const [deductionCycle, setDeductionCycle] = useState(lwfData?.deduction_cycle || "monthly");
  const [formLoading, setFormLoading] = useState(false);
  const [error, setError] = useState("");

  // UI dropdown options (backend will validate final choice)
  ...
  const handleEnable = async (e) => {
    ...
    try {
      ...
      // Backend returns ok=false for invalid cycles → show backend error
      if (res?.ok === false) {
      ...
      const backendError =
        ...
        err?.message ||
        "Failed to enable LWF. Please try again.";
  ...
  if (!enabled) {
    return (
      <div className="px-4 py-6 bg-gray-50 rounded-md">
        <h3 className="font-medium text-gray-800 mb-4">Enable Labour Welfare Fund</h3>
        ...
        <form onSubmit={handleEnable} className="space-y-4">
          ...
          <div>
            <label className="block text-gray-600 text-sm mb-1">Deduction Cycle</label>
            <select
  ...
  return (
    <div className="px-2 sm:px-4 md:px-6 w-full">
      <h3 className="font-medium text-[14px] sm:text-[15px] text-black flex items-center gap-3">
        Labour Welfare Fund{" "}
        <span className="font-normal text-xs sm:text-[12px]">(Ensures social welfare)</span>
      </h3>
      ...
      <div className="mt-3 space-y-5 text-sm text-gray-700">
        <div className="flex gap-3">
          <span className="w-[180px] text-gray-600">Employee’s Contribution</span>
      ...
      <div className="flex items-center mt-4 gap-10">
        <div className="flex flex-col">
          ...
          <span className={`text-[11px] font-medium ${enabled ? "text-green-500" : "text-red-500"}`}>
            {enabled ? "Enabled" : "Disabled"}
          </span>
        </div>

        <button onClick={onDisable} className="px-5 py-[6px] text-[12px] bg-black text-white rounded-md">
          Disable
        </button>
      </div>
    </div>
