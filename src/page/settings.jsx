...
import { useAuthStore } from "../utils/authStore"; // ✅ your Zustand store path
...
function Settings() {
  ...
  // ✅ Zustand + router hooks
  const { user, logout } = useAuthStore();
  const navigate = useNavigate();

  // ✅ Handle logout with toast + navigate
  const handleLogout = () => {
    logout();
  ...
  useEffect(() => {
    if (user) {
      const u = user.user || user; // handle nesting
      setFormData({
  ...
  return (
    <div className="flex h-screen bg-black p-3 overflow-hidden">
      ...
      <SideBar
        ...
        isCollapsed={false}
        toggleSidebar={() => {}}
        onLogout={handleLogout} // ✅ Pass logout here too
      />
